(function() {
  ...
  // rendering engine called "Shark"
  isShark = _.memoize(function () {
    var release = Meteor.release;
    return release.match(/^template-engine-preview(-\d+)?$/) ||
      // XXX this is true only in case of 0.7 being a Shark release
      parseFloat(release) < 0.7;
  });
}());
